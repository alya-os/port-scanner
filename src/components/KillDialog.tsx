...
export function KillDialog({ process, stopping, onCancel, onConfirm }: KillDialogProps) {
  ...
  return (
    <div className="dialog-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && !stopping && onCancel()}>
      <section className="kill-dialog" role="alertdialog" aria-modal="true" aria-labelledby="kill-title" aria-describedby="kill-description">
        ...
        <footer>
          <button className="secondary-button" type="button" onClick={onCancel} disabled={stopping}>Annuler</button>
